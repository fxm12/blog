		...
		if(err){
			...
			User.find(req.body,function(err,users){
				if(err){
					...
					var sex =['男','女','保密'];
					var position = ['北京市','黑龙江省','辽宁省','河北省']
					res.render('users/index',{users:users,moment:moment,sex:sex,position:position,page:page,query:req.query,str:str})
				}
			}).where(where).limit(4).skip((page.nowPage-1)*page.every)
		}
	}).where(where);
});
